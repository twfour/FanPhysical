...
日常发布使用现有 SSH 密钥发送远端部署命令，由 ECS 从 GitHub 下载当前提交：

```bash
deploy/aliyun/deploy_from_github.sh
```

运行前需先提交并推送当前版本。该方式只通过 SSH 发送短命令，不从本机上传项目压缩包；`deploy/aliyun/deploy.sh` 保留为备用上传方案。
...
先在本地 `.env` 中配置 `DEEPSEEK_API_KEY`、至少 12 个字符的 `NOTEBOOKLM_EDIT_PASSWORD`，以及另一个不同的 `LEARNING_SYNC_PASSWORD`，再安装受保护的环境变量并发布应用：
