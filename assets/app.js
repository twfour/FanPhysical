...
var physicsSoundStorageKey = "fanphysics:mechanicalEnergySound:v1";
var physicsLegacySoundStorageKey = "fanphysics:referencePlaneSound:v1";
...
function readPhysicsSoundPreference() {
  try {
    ...
    if (stored === null) {
      stored = window.localStorage.getItem(physicsLegacySoundStorageKey);
    }
    return stored === "1";
  } catch (error) {
    return false;
...
function ensurePhysicsAudio(sceneName) {
  ...
  if (!physicsSoundEnabled || !isMechanicalEnergySoundScene(targetScene) || !supportsPhysicsSound()) {
    return null;
  }
...
function updatePhysicsContinuousSound(sceneName, progress, playing) {
  if (!physicsSoundEnabled || !playing || !isMechanicalEnergySoundScene(sceneName) || currentScene !== sceneName) {
...
function playPhysicsSoundEvent(kind, intensity, sceneName) {
  var targetScene = sceneName || currentScene;
  if (!physicsSoundEnabled || !isMechanicalEnergySoundScene(targetScene) || currentScene !== targetScene) {
    return;
  }
  ...
  setPhysicsMasterGain(0.52, false);
  if (kind === "impact" || kind === "rope") {
    createPhysicsNoiseBurst(context, 0.22, kind === "rope" ? 0.12 : 0.18, 950, "lowpass", intensity);
    createPhysicsToneBurst(context, 105, 52, 0.2, 0.13, "sine", intensity, 0);
  } else if (kind === "splash") {
...
function beginPhysicsSoundPlayback(sceneName, progress) {
  if (!physicsSoundEnabled || !isMechanicalEnergySoundScene(sceneName)) {
    return;
  }
...
function setPhysicsSoundEnabled(enabled) {
  ...
  if (!physicsSoundEnabled) {
    silencePhysicsAudio();
  } else if (isMechanicalEnergySoundScene(currentScene)) {
...
function renderJsonAnimationControls(sceneName) {
  ...
  if (animation.playable) {
    ...
    play.onclick = function () {
      ...
      if (state.playing) {
        if (isMechanicalEnergySoundScene(sceneName) && physicsSoundEnabled) {
          beginPhysicsSoundPlayback(sceneName, duration > 0 ? state.time / duration : 0);
        }
      } else if (isMechanicalEnergySoundScene(sceneName)) {
        pausePhysicsSoundPlayback();
      }
      syncJsonTimeControl(sceneName);
    };
    ...
    if (isMechanicalEnergySoundScene(sceneName)) {
...
function updateJsonAnimation(dt) {
  ...
  if (isMechanicalEnergySoundScene(currentScene)) {
